...
last) write_header " List of last logged in users "; who | tail -5 ; pause ;;
esac 
}
...
2 )
	...
	read pidtokill
	kill -9 $pidtokill && echo "${txtgrn}Process terminated successfully.${txtrst}" | center || echo "${txtred}Process failed to terminate. Please check the PID and try again.${txtrst}" | centerwide
...
2 )
	...
	read rmfile
	echo "" && echo ""
	rm $rmfile && echo "${txtgrn}File removed successfully.${txtrst}" | center || echo "${txtred}File removal failed.${txtrst}" | center
	echo "" && echo "${txtcyn}(press ENTER to continue)${txtrst}" | center && read
;;
...
3 )
	...
	read mkdirdir
	echo "" && echo ""
	mkdir $mkdirdir && echo "${txtgrn}Directory $mkdirdir created successfully.${txtrst}" | centerwide || echo "${txted}Failed to create directory.${txtrst}" | center
	echo "" && echo "${txtcyn}(press ENTER to continue)${txtrst}" | center && read
;;
...
4 )
	...
	read rmdirdir
	echo "" && echo ""
	rm -rf $rmdirdir && echo "${txtgrn}Directory $rmdirdir removed successfully.${txtrst}" | centerwide || echo "${txtred}Failed to remove directory. Please ensure directory is empty.${txtrst}" | centerwide
	echo "" && echo "${txtcyn}(press ENTER to continue)${txtrst}" | center && read
;;
...
5 )
	...
	cat $symloutfile && clear && echo "" && echo "${txtgrn}Symbolic link created successfully at $symloutfile${txtrst}" | centerwide || echo "${txtred}Failed to create symbolic link. Please check paths and filenames and try again.${txtrst}" | centerwide 
	echo "" && echo "${txtcyn}(press ENTER to continue)${txtrst}" | center && read
;;
...
6 )
	...
	read chowngroup
	echo "" && echo ""
	chown $chownuser $chownfile && chgrp $chowngroup $chownfile && echo "${txtgrn}Ownership of $chownfile changed successfully.${txtrst}" | centerwide || echo "${txtred}Failed to change ownership. Please check if user, group and file exist.${txtrst}" | center
	echo "" && echo "${txtcyn}(press ENTER to continue)${txtrst}" | center && read
;;
...
9 ) 
	...
	case $pressmethod in
		...
		3 )
			compress -f $pressfile && echo "${txtgrn}File compressed successfully.${txtrst}" | center || echo "${txtred}File failed to compress.${txtrst}" | center
		;;
...
#Purpose - Scan for open ports as well as open and closing selected ports
function port_info(){

echo "${txtcyn}(please enter the number of your selection below)${txtrst}"
echo ""
...
echo "9. ${txtpur}Return to main menu${txtrst}"
echo ""

read portmenuchoice
case $portmenuchoice in

1 )
	echo "Please enter the ${txtcyn}IP Address${txtrst} that you want to run an Nmap scan on: "
	echo "${txtcyn}(type localhost or 127.0.0.1 to scan this computer)${txtrst}"
	read scanAddress
	echo ""
	nmap $scanAddress || echo "${txtred}Nmap failed. Is Nmap not working?${txtrst}"
	pause
;;
2 )
